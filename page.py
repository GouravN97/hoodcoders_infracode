...
def main():
    ...
    if submitted:
        ...
        input_payload = {
            ...
            "Funding_Approved": float(funding_approved),
            "Funding_Received": float(funding_received),
            "Inflation_Rate": float(inflation_rate),
            "Inflation_Index": float(inflation_index),
            "Feasibility_Score": float(feasibility),
            "Sustainability_Score": float(sustainability),
            "Public_Benefit_Score": float(public_benefit),
            "Stakeholder_Priority": int(stakeholder_priority),
            "Funding_Delay_%": float(funding_delay)
        }
